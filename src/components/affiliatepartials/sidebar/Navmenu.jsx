...
import { resellerLogout, switchProfile } from "../../../constant/apiRoutes";
...
const Navmenu = ({ menus }) => {
  ...
  const handleLogout = async () => {
    try {
      const { data } = await axios.get(resellerLogout, {
    ...
    <>
      <ul>
        ...
        <li
          className="single-sidebar-menu"
          onClick={() =>
            handleSwitch({ switchTo: profileSwitchTypes.seller, type: 2 })
          }
        >
          <div className="menu-link">
            <span className="menu-icon flex-grow-0">
            ...
            <div className="text-box flex-grow">Switch to Shop</div>
          </div>
        </li>
        <li
          className="single-sidebar-menu "
