...
import {
  getSellerPropertyVideo,
  insertSellerPropertyVideo,
...
const AddVideo = () => {
  ...
  const getData = async () => {
    try {
      ...
      if (data.error === false) {
        setData((prev) => ({
          ...prev,
          video: data.data.property[0].video,
          thumbnail: data.data.property[0].thumbnail,
        }));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.error === false) {
       ...
       navigate(`/dashboard/seller/property/${id}/amenities`)
      }
    } catch (error) {
      toast.error(error.message);
    } finally{
  ...
  return (
    <>
      <Card title={"Property Video"} headerslot={<BackButton />}>
