            ...
            <Tab.List className="grid grid-cols-2 md:grid-cols-1 mb-5 gap-2">
              {items.map((item, i) => (
                <Tab key={i} as={Fragment}>
                  {({ selected }) => (
                    <button
                      className={`w-full col-span-1 text-left text-sm font-medium md:block inline-block md:mb-4 last:mb-0 capitalize ring-0 foucs:ring-0 focus:outline-none px-6 rounded-md py-2 transition duration-150
            ${
              selected
                ? "text-white bg-main "
