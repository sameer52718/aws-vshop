...
const AddFiles = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        if (data.error === false) {
          if (data.data.vehicle[0].files) {
            const parsedData = JSON.parse(data.data.vehicle[0].files);
  ...
  const handleDelete = async (e,file) => {
    ...
    try {
      ...
      const {data} = await axios.post(deleteVehicleFiles,{id,file},{headers:{Authorization:token}})
      if (data.error === false) {
        toast.success(data.message)
      }
    } catch (error) {
