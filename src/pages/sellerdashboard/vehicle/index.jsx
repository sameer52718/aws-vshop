...
import { getSellerVehicle, updateVehicleStatus } from "../../../constant/apiRoutes";
import { useSelector } from "react-redux";
import { VehicleColumn } from "../../../constant/columns";
import Tooltip from "../../../components/ui/Tooltip";
import DashboardLoader from "../../../components/DashboardLoader";

const Vehicle = () => {
  const { token } = useSelector((state) => state.auth);
  const columns = useMemo(() => VehicleColumn, []);
  ...
  useEffect(() => {
    const getData = async () => {
      setIsLoading(true);
      try {
        const { data } = await axios.get(getSellerVehicle, { headers: { Authorization: token } });
        if (data.error === false) {
          setData(data.data.vehicle ?? []);
        } else {
            ...
            return (
              <>
                <div className="flex space-x-2">
                  ...
                  <Tooltip content={"Features"} placement="top" arrow animation="shift-away" theme="primary">
                    <button
                      className="action-btn"
                      type="button"
                      onClick={() => navigate(`${row.cell.row.original.id}/features`)}
                    >
