...
import { getSellerFollow, imgUrl } from "@/constant/apiRoutes";
...
const Follow = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setIsLoading(true);
        const { data } = await axios.get(getSellerFollow, {
          ...
          <Tab.Panels className="w-full">
            <Tab.Panel>
              <Card title={"Followers"}>
                <div className="md:space-y-0 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {data?.followers?.map((item) => (
                    <Card
                      className="!bg-gray-50 col-span-1 relative"
                      key={item.url}
                    >
                      <div className="flex justify-between">
                        ...
                        <div
                          ...
                          } flex items-center gap-1 text-[10px] xs:text-xs px-1 xs:px-2 py-[2px] xs:py-1 capitalize text-white rounded-md tracking-wide absolute top-1 right-1`}
                        >
                          <img
