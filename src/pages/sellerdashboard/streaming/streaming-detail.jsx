...
import { createSellerStreaming, getSellerCategories } from "../../../constant/apiRoutes";
...
const StreamingDetail = ({ setActive }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        if (data.error === false) {
          setOptions(data.data?.category?.flat(1));
        } else {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const { data: res } = await axios.post(createSellerStreaming, body, {
