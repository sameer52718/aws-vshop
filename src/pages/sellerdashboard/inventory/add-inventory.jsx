...
import { insertProductInventory,getSellerProducts,productThumbnailRoute ,getProductVariations  } from "@/constant/apiRoutes";
...
const AddInventory = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        setIsLoading(true);
        const { data } = await axios.get(getSellerProducts, {
  ...
  useEffect(() => {
    const getProduct = async (product) => {
      try {
        setColor([])
        setSize([])
        const {data} = await axios.post(getProductVariations,{url:product.url},{headers:{Authorization:token}})
        if (data.error=== false) {
          const {color,size} = data.data.variation.at(0)
