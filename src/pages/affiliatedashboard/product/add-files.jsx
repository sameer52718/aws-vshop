...
import {
  deleteResellerProductFiles,
  getResellerProductFiles,
  insertResellerProductFiles,
  productFilesRoute,
...
const AddFiles = () => {
  ...
  useEffect(() => {
    const getData = async () => {
      try {
        ...
        if (data.error === false) {
          if (data.data.product[0].files) {
            const parsedData = JSON.parse(data.data.product[0].files);
  ...
  const handleDelete = async (e,file) => {
    ...
    try {
      ...
      const {data} = await axios.post(deleteResellerProductFiles,{url,file},{headers:{Authorization:token}})
      if (data.error === false) {
        toast.success(data.message)
      }
    } catch (error) {
